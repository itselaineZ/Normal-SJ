...
int menu(const HANDLE hout) {
	...
	cout << "5.用箭头键控制上下左右(边界停止，演示HPKM可移动的错误，此项不需要实现)\n";
	cout << "6.用箭头键控制上下左右(边界回绕，演示HPKM可移动的错误，此项不需要实现)\n";
	cout << "0.退出\n";
	return _getch() - '0';
}

void move_cursor(const HANDLE hout, bool lft_arw, bool arw, bool revlv) {
