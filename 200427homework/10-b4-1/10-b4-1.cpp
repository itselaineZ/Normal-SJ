...
public:
	...
	const matrix<T, row, col> operator + (const matrix<T, row, col>& x);
...
template<class T, int row, int col>
const matrix<T, row, col> matrix<T, row, col>::operator + (const matrix<T, row, col>& x)
{
	matrix<T, row, col> rt;
	for (int i = 0; i < row; ++i)
		for (int j = 0; j < col; ++j)
			rt.value[i*col+j] = this->value[i*col+j] + x.value[i*col+j];
	return rt;
}
